...
\textbf{Grupo} nr. & 106
\\\hline
93189 & Bernardo Emanuel Magalhaẽs Saraiva
\\
...
\start
    |out . in = id|
%
\just\equiv{ Def in , Fusao-+}
%
 [out . \underline{X}, out . [N , [Bin, \hat{Un}]]] = id
%
\just\equiv{ Fusao-+}
%
[out {.} \underline{X}, [out . N,out .[Bin, \hat{Un}]]] = id
%
\just\equiv{ Fusao-+}
%
[out . \underline{X}, [out . N ,[out . Bin, out . \hat{Un}]]] = id
%
...
%
 ...
 |out . const X= i1|\\
 %
 |[out . N, [out . Bin, out .  uncurry Un]] = i2|
 % 
   \end{array}
 \right.
 %
...
\xymatrix@@C=2cm{
    |ArvExp|
           ...
           \ar[r]^-{|out|}
&
    |1 + (A + (BinOP >< (ExpAr A >< ExpAr A))) + (UnOp >< ExpAr)|
           \ar[d]^{recExpAr |cataNat g|}
\\
     |B|
&
...
---
g_eval_exp a = either (g1) (g2)
                where g1 = const a
                      g2 = either(id) (g3)
                      g3 = either (g4) (g5)
                      g4 (Sum,(a,b)) = a + b 
                      g4 (Product,(a,b)) = a * b
                      g5 (Negate, a) = negate a
                      g5 (E, a) = expd a
---
\end{code}
...
\xymatrix@@C=0.01cm{
    |ExpAr A|
           \ar[r]^-{|inExpAr|}
&
    |1+(A+(BinOP(ExpArA >< ExpAr A)))+(UnOp >< ExpAr)|
           \ar[d]^{|recExpAr ana clean|}
\\
     |ExpAr A|
          \ar[u]_-{|ana clean|}
          \ar[d]^-{|outExpAr|}
...
clean = outExpAr . y . outExpAr where
        y = either (const X) num_ops
        num_ops = either N ops
        ops = either bin (uncurry Un) 
...
\xymatrix@@C=2cm{
    |ExpAr A|
           \ar[d]_-{|cataNat sd_gen|}
           \ar[r]^-{|outExpAr|}
&
    |1 + (A + (BinOP >< (ExpAr A >< ExpAr A))) + (UnOp >< ExpAr A)|
           \ar[d]^{|recExpAr (cataNat sd_gen)|}
\\
     |ExpAr A >< ExpAr A|
          \ar[d]_-{|p2|}
&
     |1+A+BinOP><((ExpAr A >< ExpAr A)><(ExpAr A >< ExpAr A)) + UnOp><(ExpAr A >< ExpAr A)|
           \ar[l]^-{|sd_gen|}
\\
    |ExpAr A|
}
...
sd_gen = split funcao derivada where 
    funcao = either (const X) num_ops
    num_ops = either N ops 
    ops     = either bin un
    bin(op, ((a,b),(c,d))) = Bin op a c
    un(unop,(a,b)) = Un unop a
    
    derivada = either (const (N 1)) num_opsd
    num_opsd = either (const (N 0)) opsd
    opsd     = either bind und
    bind(Sum, ((a,b),(c,d))) = Bin Sum b d
    bind(Product, ((a,b),(c,d))) = Bin Sum (Bin Product a d) (Bin Product b c)
    und(E,(a,b)) = Bin Product (Un E a) b
    und(Negate,(a,b)) = Un Negate b
\end{code}

\begin{eqnarray*}
\xymatrix@@C=2cm{
    |ExpAr A|
           \ar[d]_-{|cataNat ad_gen|}
           \ar[r]^-{|out|}
&
    |1 + (A + (BinOP >< (ExpAr A >< ExpAr A))) + (UnOp >< ExpAr A)|
           \ar[d]^{|recExpAr (cataNat ad_gen)|}
\\
     |A >< A|
          \ar[d]_-{|p2|}
&
...
ad_gen v = either g1 (either g2 (either g3 g4)) where 
        g1 = const(v,1)
        g2 a = (a, 0)
        g3 (oper,((a,b),(c,d))) = if (oper == Product) then (a*c,a*d + b*c) else (a+c,b+d)
        g4 (oper,(a,b)) = if (oper == E) then (expd a,(expd a) * b)  else (negate a,negate b)
...
\begin{eqnarray*}
\start
|catdef n = div (fac((2*b))) ((fac((n+1))*(fac n)))|
%
\just\equiv{}
%
 |catdef (n+1) = div (fac((2*(n+1)))) (((fac((n+1)+1)) * fac((n+1))))|
%
\just\equiv{trivial}
%
|catdef (n+1) = div (fac((2*n+2)))  (((fac((n+2))) * (fac((n+1)))))|
%
\just\equiv{(n+1)! = (n+1)* n! }
%
|catdef (n+1) = div (((2*n+2) * (2*n+1) * (fac((2n))))) (((n+2) * (fac((n+1))) * (n+1) * fac ((n))))|
%
\just\equiv{def catdef n}
%
|catdef (n+1) = div ((2*(n+1) * (2*n+1)))  (((n+2) * (n+1)))  * catdef n|
%
\just\equiv{corta n+1}
%
|catdef (n+1) = div ((4*n+2)) ((n+2)) * catdef n|
%
\just\equiv{(e n = (4 * n + 2)   ,   h n = (n + 2)}
|catdef (n+1) = div (e n) (h n) * catdef n|
%
\just\equiv{calcular e (n+1) , h (n+1)}
  \left\{
   \begin{array}{ll}
      e (n+1) = 4 * (n+1) + 2\\
      | h (n+1) = (n + 1) + 2|
...
\begin{code}
inic = (1,2,2)
loop (cat,e,h) = ((cat * e) `div` h,4 + e,1+h)
prj (cat,e,h) = cat
\end{code}
por forma a que
\begin{code}
...
calcLine = cataList h where
   h = either p1 p2 where
    p1 () = const nil
    p2 (d,f) [] = nil
    p2 (d,f) (x:xs) = z1 where
...
\start
    | avg_aux = cataNat[b,q]|
%
\just\equiv{avgaux = \textless avg,length \textgreater}
%
 { \textless avg,length \textgreater} = |cataNat  [b,q]|
%
\just\equiv{b e q são splits}
%
{ \textless avg,length \textgreater} = |cataNat  [<b1,b2>,<q1,q2>]|
%
\just\equiv{lei da troca}
%
|<avg,length> = cataNat  (<[b1,q1],[b2,q2]>)|
%
\just\equiv{Fokkinga}
  \left\{
   \begin{array}{ll}
      |avg . in = [b1,q1] . F<avg, length>|\\
      |length . in = [b2,q2] . F<avg,length>|
  \end{array}
\right.
%
\just\equiv{in = [singl,cons], F f = id + id x F}
  \left\{
   \begin{array}{ll}
      |avg . [singl, cons] = [b1,q2] . (id +(id >< <avg, length>))|\\
      |length . [singl, cons] = [b2,q2] . (id +(id >< <avg, length>))|
  \end{array}
\right.
%
\just\equiv{absorção +, Fusão +}
  \left\{
   \begin{array}{ll}
      |[avg . singl, avg . cons] = [b1 . id, q1 . id >< <avg,length>]|\\
      |[length . singl, avg . cons] = [b2 . id, q1 . id >< <avg,length>]|
  \end{array}
\right.
% 
  \just\equiv{eq-+}  
  \left\{
   \begin{array}{llll}
      |avg . singl = b1 . id|\\
      |avg . cons = q1 . (id >< <avg,length>)|\\
      |length (singl a) = b2 a|\\
      |length cons(a,b)) = q2 (id >< <avg,length>) (a,b)|
  \end{array}
\right.
%
  \just\equiv{def-comp, igualidade extensional , natural-id}  
  \left\{
   \begin{array}{llll}
      |avg (singl a) = b1 a|\\
      |avg (cons(a,b)) = q1 (id >< <avg,length>) (a,b)|\\
      |length (singl a) = b2 a|\\
      |length cons(a,b)) = q2 (id >< <avg,length>) (a,b)|
  \end{array}
\right.
%
\just\equiv{def singl a = [a], cons(a,b) = (a:b), def-x, def-split}  
  \left\{
   \begin{array}{llll}
      |avg [a] = b1 a|\\
      |avg (a:b) = q1 (a,(avg b,lengthb)|\\
      |length[a] = b2 a|\\
...
\begin{code}
inListNotEmpty = either singl cons

outListNotEmpty [a]    = i1 (a)
outListNotEmpty (a:x) = i2(a,x)

cataListNotEmpty g   = g . recListNotEmpty (cataListNotEmpty g) . outListNotEmpty 

recListNotEmpty  f   = id -|- id >< f  

avg = p1.avg_aux
\end{code}

\begin{code}
avg_aux = cataListNotEmpty (either b q) where 
    b = split b1 b2
    b1 a = id a
    b2 a = 1
    q = split q1 q2
...
\right.
%
\just\equiv{in = [Leaf,Fork], F f = id + (f x f)}
  \left\{
   \begin{array}{ll}
      |avg . [Leaf, Fork] = [b1,q1] . (id + (<avg, length> >< <avg, length>))|\\
      |length . [Leaf, Fork] = [b2,q2] . (id +(<avg, length> >< <avg, length>))|
  \end{array}
\right.
%
\just\equiv{absorção +, Fusão +}
  \left\{
   \begin{array}{ll}
      |[avg . Leaf , avg . Fork] = [b1 . id, q1 . (<avg, length> >< <avg, length>)]|\\
      |[length . Leaf , length . Fork] = [b2 . id, q2 . (<avg, length> >< <avg, length>)]|
  \end{array}
\right.
% 
  \just\equiv{eq-+, def-comp, def-split, def-x, igualdade extensional}  
  \left\{
   \begin{array}{llll}
      |avg(Leaf x) = b1 x|\\
      |avg(Fork ((LTree a, LTree b)) = q1 ((avg LTree a,length LTree a), (avg LTree b,length LTree b) )|\\
      |length (Leaf  x) = b2 x|\\
      |length (Fork ((LTree a, LTree b)) = q2 ((avg LTree a,length LTree a), (avg LTree b,length LTree b) )|
  \end{array}
\right.
%
\end{eqnarray*}
